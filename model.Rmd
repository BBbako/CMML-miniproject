...
run_trial_place <- function(weights0, Wmult, sigma_pc, sigma_ac, PC_x, PC_y, Vdecay, ac_const, beta, etdecay, lrate, discf, noise, platform_x, platform_y, starting_x, starting_y, speed, wall_pun) {
  ...
  while ((track_x[length(track_x)] - platform_x)^2 + (track_y[length(track_y)] - platform_y)^2 > platform_radius^2)
  {
    ...
    Q <- AC_activation
    Q[is.na(Q)] <- 0                      # 防止 NA
    Q <- Q - max(Q, na.rm = TRUE)        # 数值稳定的 softmax

    ACsel <- exp(beta * Q)
    ACsel[is.na(ACsel)] <- 0             # 进一步防止异常值
    ACsel <- ACsel / (sum(ACsel) + 1e-8) # 避免除以 0
...
# 后续 pairwise 对比（每一天比较 Place vs Distance）
...
```{r}
experimental_data <- read_excel("D:/data/WMdataLong.xlsx")
colnames(experimental_data)[1] <- "Variable"
# 定义变量映射：latency (1), target quadrant time (2), opposite quadrant time (10)
behavior_vars <- list(
  latency = 1,
  target = 2,
  opposite = 10
...
# 筛选条件：只保留 variable platform + C57BL/6
...
# 从实验数据中提取 latency（Variable == 1）并汇总
...
# 提取 Target Quadrant Time（Variable == 2）
exp_target <- data_c57 %>%
  filter(Variable == 2) %>%
  group_by(Day) %>%
  summarise(
...
# 提取 Opposite Quadrant Time（Variable == 10）
